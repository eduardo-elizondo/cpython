...
PyObject_FromPyGCObject(PyGCObject *gcob)
{
    if (gcob->gcob_ref == NULL)
        gcob->gcob_ref = PyGCProxyObject_New(gcob);
...
PyGCObject_FromPyObject(PyObject *ob)
{
    if (PyGCRef_CheckExact(ob))
        return ((PyGCProxyObject *)ob)->proxy_real;
    return PyGCProxyObject_New2(ob);
}
...
proxy_repr(PyObject *op)
{
    ...
    return PyObject_FromPyGCObject(PyGCObject_Repr(proxy->proxy_real));
}
